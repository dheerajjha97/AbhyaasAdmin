...
export const NotesEngineView: React.FC<NotesEngineViewProps> = ({
  ...
  // Auto-generated target filename in GitHub
  const autoGeneratedFilename = `data/notes/${classId.replace('-', '')}_${subjectId}_chap${chapterNumber}_notes.json`;
  const [targetFilename, setTargetFilename] = useState<string>(autoGeneratedFilename);
  const [isPathManuallyEdited, setIsPathManuallyEdited] = useState<boolean>(false);

  useEffect(() => {
    if (!isPathManuallyEdited) {
      setTargetFilename(autoGeneratedFilename);
    }
  }, [autoGeneratedFilename, isPathManuallyEdited]);
  ...
  return (
    <div className="space-y-4">
      ...
      <NotesMetaHeader
        ...
        setTargetFilename={handleCustomPathChange}

...
export const GitHubPushSyllabusView: React.FC<GitHubPushSyllabusViewProps> = ({
  parsedResult,
  targetFilename,
  setTargetFilename,
  branch,
  ...
  const handleExecutePush = async () => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        setStatusMessage(
          `Successfully committed syllabus "${targetFilename}" to ${repoOwner}/${repoName}@${branch} (Commit: ${result.commitSha})`
        );
      } else {
        setPushStatus('error');
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm space-y-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold text-slate-900">
                Step 4: Push Syllabus to GitHub Repository
              </h3>
            ...
            <p className="text-xs text-slate-500">
              Commit syllabus structure directly to your data repository for mobile apps
            </p>
          </div>
        ...
        {pushStatus === 'success' && (
          <div className="p-3.5 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-900 text-xs flex items-start gap-2.5 animate-in fade-in">
            ...
            <div className="space-y-1 flex-1">
              <div className="font-bold">Syllabus Committed Successfully!</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 pt-1">
          ...
          <div className="space-y-1.5 md:col-span-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full h-10 pl-3 pr-20 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono text-slate-900 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none"
              />
              <button
                type="button"
          ...
          <div className="space-y-1 md:col-span-2">
            ...
            <input
              ...
              className="w-full h-9 px-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-800 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5 pt-2 border-t border-slate-100">
          ...
          >
            ...
            <span>{isPushing ? 'Committing Syllabus to GitHub...' : 'Push Syllabus to GitHub Now'}</span>
          </button>
        </div>
      </div>
      ...
      {pushHistory.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-slate-800">Syllabus Synchronization Log</h4>
          </div>
              ...
              <div key={item.id} className="py-2.5 flex items-center justify-between gap-3">
                <div className="space-y-0.5 min-w-0">
                  ...
                  <div className="text-[11px] text-slate-500 font-mono flex items-center gap-2 truncate">
                    ...
                    <span>•</span>
                    <span className="text-emerald-700 font-bold">SHA: {item.commitSha}</span>
                  </div>
                </div>
            ...
            {isLoadingRepos ? (
              ...
              <div className="max-h-60 overflow-y-auto divide-y divide-slate-100 text-xs">
                {availableRepos.map((repo) => (
                  ...
                  >
                    <span className="font-mono text-slate-800 font-semibold group-hover:text-emerald-700">
                      {repo.fullName}
                    </span>
                    {repo.isPrivate && (
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-5 space-y-3.5 shadow-2xl border border-slate-200 animate-in zoom-in-95">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5"
              >
